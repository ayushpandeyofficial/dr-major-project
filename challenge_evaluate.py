...
if __name__ == "__main__":
    ...
    # Calculate val_labels
    val_labels = all_labels  # Assuming all_labels contain the validation labels

    # Select 120 images from each class for validation
    val_subset_indices = []
    classes = torch.unique(torch.tensor(val_labels)).numpy()

    for label in classes:
        indices = (torch.tensor(val_labels) == label).nonzero(as_tuple=True)[0]
